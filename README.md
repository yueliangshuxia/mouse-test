...
GitHub Pages **发不了 COOP/COEP 响应头**,而这两个头决定了 `performance.now()` 的精度是 5µs
还是 100µs。`public/coi-serviceworker.js` 用 service worker 在 `dist/` 这一侧把响应头补上,
代价是**首次进入需要重载一次**(跨域隔离是文档的属性,导航那一刻就定死了)。它只改同源响应、
不缓存任何东西;注册失败(隐私模式、老浏览器)就静默退回 100µs,页面顶部如实告警。
本地 `astro dev` 走 `astro.config.mjs` 的 `server.headers`,不需要这个。

要那条路从根上正常,**换一个能自定义响应头的托管方**仍然是最干净的解法。另外 `github.io`
在国内访问不稳定且不能备案,所以 Pages 只适合当预览环境。
