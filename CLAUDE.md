...
| `src/lib/mini-devices.ts` | 首页卡片的**装置运行时**:惰性挂载 + `idle→running→done` 状态机 + 五个工厂 + 模式换挡与结算冷却 | 真机手测 |
...
- **`wide` 是"这张卡够宽时跨两格"的钩子,只有集合体该写它。** 和 `covers` 一样是一张卡特有的事实,所以有自己的字段,而**不就地判 `kind === 'mouse-buttons'`** —— 那样以后再加一张放鼠标的卡会**静默地**跟着跨格。目前八张里只有合并卡那一条写了。**"要不要"在 `mini-cards.ts`,"什么时候生效"在 `global.css` 的 `@container` 里**,后者是渲染之后才有的事实,别把这个判断挪进模板。`tests/mini-cards.test.ts` 钉住**至多一张卡声明 wide**,而且声明了的卡必须真的有 `covers`:两张 wide 在 3 列下是 10 个格子铺 4 行、末尾空两格,那正是这条要挡的东西。
...
**八张诊断卡,5 张能放装置,3 张放不进 —— 而卡片是 8 张、诊断工具是 10 个。**(趣味工具那一节根本没有卡片,不进这个分类。)判据只有一句:**卡片里那个数,和那一页的数是不是同一件事?**

- 放得进:按键(这一张同时顶了**长按拖拽**和**双击**) / 滚轮 / CPS / 轨迹 / 回报率。
- 放不进:**DPI、加速度**(要沿尺子推固定物理距离,卡片里没有那把尺子)、**键盘**(首页的按键会滚动页面、撞浏览器快捷键)。这三张卡的装置槽里放的是一段**说明**,不是空框 —— 说清楚为什么放不进来,再指向那一页。
- **前三张并成一张是用户定的**,不是没来得及做。理由是那张卡量到的三样东西(按下哪个键 / 按住多久 / 双击间隔)本来就是同一串 pointer 事件推出来的,拆成三张卡等于让用户按三种方式按同一个键。**被顶掉的两页照旧存在**:小注末尾那行「独立页面:」就是它们的入口,少那行就是两个没有首页入口的孤页。
- **合并卡上还多一个滚轮方向,而它不算"顶掉滚轮那一页"。** `covers` 里**没有** `scroll-test`,「滚轮」那张卡照旧存在(格数、最近 10 次的带子都在那边)。判据是上面那句"卡片里那个数和那一页的数是不是同一件事":**方向上卡,格数不上卡** —— 方向是滚动唯一不需要任何假设的那一位,格数要过 `normalizeWheelDelta` 那层启发式换算。所以卡名写了滚轮、`covers` 没写,两者不矛盾:卡名说的是"这张卡会响应滚动",`covers` 说的是"哪一页被它顶掉了"。**把 `scroll-test` 加进 `covers` 会让单测的"恰好覆盖一次"当场红**(那一页已经有自己的卡)。
  画法也刻意不是一行读数:轮子亮起来 + 一支箭头上/下,两样都长在**那张鼠标图**上(`MOUSE_SVG` 的 `data-wheel-dir`,状态挂在 `.mini-mouse[data-wheel]`)。理由和格数同源 —— 方向不是有量纲的量,给它一格读数等于把它说成一件可以比大小的东西。
...
- **卡底那行小注是折进来的「关于测量精度」**,是**真实可见的正文**,不是 tooltip、不是折叠面板。收起来等于把全站的主张从首页删掉。**这两层文字不在同一个地方渲染,别找错文件**:卡片里只渲染 `spec.note`(这一项特有的那一句,`index.astro` 的 `.mini-card__note`);`CONFIDENCE_NOTE` 那四档骨架**只在网格下面渲染一次**,是一份 `<dl class="confidence-key">`,靠可信度徽章和上面八张卡对上。改「四档各是什么意思」改那份 `<dl>`,改「这一项有什么限制」改 `mini-cards.ts` 的 `note` —— **卡里没有骨架**,按老描述去卡里找会扑空。
...
- **装置是惰性的**:装载时一个采样器都不建。`DEVICES[kind].wake` 声明"哪种事件算开始用",用户碰到装置面才调工厂。**不能一律三种事件都收** —— 一律收 `pointermove` 的话,鼠标从页面上扫过就挂起一串采样器和画布。采样时 `capacity` 要传 `20000`(默认 120000 ≈ 2MB 一份),`capture` 保持 `true`。
  **`wake` 是可以有多条的,合并卡就是 `['pointerdown', 'wheel']`。** 判据是"这个装置能不能被这种事件唤起" —— 合并卡上滚轮是一等公民,只挂 `pointerdown` 的话空态下滚滚轮什么都不会发生,而"空态"正是它第一次被用到的样子。这一条**只在工厂自己能处理 seed 时才对**:`mouseButtonsMini` 的结尾分成两支(`seed instanceof WheelEvent` 走 `onWheel`、否则走 `asPointer`),少一支就是"第一次交互白丢一次"。
  加 `wheel` 的代价只有一条:卡片装置面上的滚动不再带动页面(那是想要的,和「滚轮」那张卡同一条理由)。**实测验过它没有越界**:同一段合成滚轮在四个落点上读事件自己的 `defaultPrevented`(这台 headless 里页面根本滚不动 —— `Input.dispatchMouseEvent` 的 `mouseWheel` 和 `Input.synthesizeScrollGesture` 量到的 `scrollY` 恒为 0,所以判据只能换成"事件有没有被取消"):装置面 **true**、同一张卡的小注 **false**、网格外的首页小字 **false**、滚轮卡装置面 **true**。
...
- **方向带是零高度成本的,但那是量出来的,不是设计出来的。** 它挂在**已经存在的那一行读数**里(`.mini-read` 的第三个 `__item`),而那一行本来就比 18px 的带子高,所以首页总高**一格没涨**:加带子前后实测同为 1440 下 2142px、420 下 3622px(逐字节相同)。**要再往读数行里塞东西,先想清楚会不会把这行顶高** —— 顶高了是八张卡一起涨。带子本身 68px = 10 格 × 5px + 9 个间距 × 2px。
...
- 实测(1440,`probe-align.mjs` 量「读数底 → 小注顶」):滚轮 **118px**、CPS **102px**、轨迹 70px、DPI **138px**;`按键 / 回报率 / 加速度 / 键盘` 都是 9px(它们本来就是那一行最高的,等于没变)。**同一行里小注的顶边仍然不齐**(小注本身行数不同),齐的是**卡片的上下边**。
- **窄屏(≤760px)只有一列,一张卡独占一行,`stretch` 与 `start` 没有分别** —— 这个改动只在两列及以上的宽度上有可见效果(实测 420 下八张卡的缝隙全是 9px)。

**合并卡跨两格:一个容器、一个阈值,而且只能是容器查询。**
...
- 那一条 `.mini-card--wide .mini-read { justify-content: space-between }` **只是看图定的,没量过** —— 645px 里四个小读数挤在左边会显得整行偏着。撤掉它不影响别的。
...
- **窄屏不是把 120px 缩成 88px,是两段。** 760px 那个**既有**断点里写的是:空态 `height: auto; min-height: 0; padding: 10px`(收成行高,约 42px),**只有 `.mini-device--running` 才给回 88px**。也就是说收掉的只是"还没测"那块空面积,测量面板一开测就长回去 —— 八张卡省下的那几百像素全在这里。**别把它"还原"成窄屏统一 88px 或 120px**:空态一旦占住高度,窄屏首页的高度预算立刻崩。这里**不新增断点**,用的就是那一个。
...
- **不是 `suppressWorkbenchDefaults()`。** 首页是一篇正文,八张卡只是它中间的一节;在那上面拦截**整页**右键,连"复制"都没了,那不是把工作台摘干净,是对读者耍横。两个函数都收一个"作用域"参数,判据和工具页共用同一份实现。
...
- **归属要连"在不在作用域里"一起在按下那一刻定下来(`pressInScope`),不能在 `contextmenu` 时现算。** 这是同一个漏网的另一个成因,而且是**上线之后被用户报出来的**:首页卡片装置是惰性挂载的,右键按在**还没碰过**的装置面上时那一下 `pointerdown` 会唤醒它,而 `start()` 里那句 `surface.replaceChildren(live)` 把"按下时落点那个节点"**从 DOM 里摘掉了**;`Node.contains()` 对**游离节点**恒为 `false`(它走的是祖先链,不在树里的节点谁也够不着),于是现算的判据在那一刻判成"不在作用域",拦截静默失效 —— 第一下右键照旧弹菜单。实测复现:同一张卡,装置已经跑着时拦得住(`detached: false`),空态被右键唤醒的那一次拦不住(`phase: "running"`、`detached: true`、`blocked: false`)。**归属是按下那一刻的事实,就该在那一刻记**;后面那个节点还在不在,和"用户是在哪儿按的"没有关系。`pressOrigin` 仍然要留着,因为放行规则的另一半(「从控件上按下、又还在同一个控件里松开」)要看的是**松手时**指针还在不在它里面。
  修完的实测(合成 `pointerdown` + `contextmenu`,含空态那一档):空态合并卡 **blocked: true**、空态 CPS **true**、跑起来的合并卡 **true**、空态滚轮卡 **true**、卡片描述 **true**、标题链接 **false**、小注里的「独立页面」链接 **false**、网格外 `<h1>` **false**、装置面上的中键 `mousedown` **true**。
